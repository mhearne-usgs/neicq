...
MONSTER_QUERY = '''select /*+ FIRST_ROWS(1) */
 nvl(substr(el.huidevent,1,8), 'NaN')  || ',' EventCode, nvl(substr(el.teventcreated - aoInitial.torigin, 1, 10), 'NaN')  || ',' tDetectLatency, 
	   ...
	   decode(InitRel.idBHPellet, NULL, 'FALSE', InitRel.idBHPellet, 'TRUE','MAYBE') || ','  QuickRev, 
       ...
       decode(BHPMTOnly.idBHPellet, NULL, 'FALSE', BHPMTOnly.idBHPellet, 'TRUE','MAYBE') || ','  Q2uickRev, 
       ...
       nvl(substr(iQCEventDataQuality,1,3),'NaN')|| ',' iQCEventDataQuality,
       nvl(substr(IQCTIMESHIFT,1,3),'NaN')|| ',' IQCTIMESHIFT,
 ...
 from PDEHydra p,
	  ...
	  ALL_BHPELLETDESCS BHPFinal,
	  ALL_AUTHOR_INFO aPOFinal
 where p.idAOInitial = aoInitial.idActualOrigin(+)
   ...
   and p.idBHPF = BHPFinal.idBHPellet
   and BHPFinal.idAuthorOrigin = aPOFinal.idauthor
'''

MONSTER_QUERY_OLD = '''select /*+ FIRST_ROWS(1) */
 nvl(substr(el.huidevent,1,8), 'NaN')  || ',' EventCode, nvl(substr(el.teventcreated - aoInitial.torigin, 1, 10), 'NaN')  || ',' tDetectLatency, 
       nvl(substr(p.OT,1,13), 'NaN') || ',' tOriginPDE, nvl(substr(round(p.Mag,1),1,10), 'NaN') || ','  MagPDE,
       nvl(substr(p.Lat,1,10), 'NaN') || ','  dLatPDE, nvl(substr(p.Lon,1,10), 'NaN') || ','  dLonPDE, 
	   nvl(substr(p.Depth,1,11), 'NaN') || ','  dDepthPDE, nvl(substr(NULL,1,10), 'NaN') || ','  iUsedPhPDE,
...
QUERY_COLUMNS = ['EVENTCODE','TDETECTLATENCY','TORIGINPDE','MAGPDE','DLATPDE','DLONPDE','DDEPTHPDE',
                 ...
                 'SINSTFIR','TFIRSTEXTP','TFIRSTATWC','TFIRSTPTWC','TFIRSTATWCINT','TFIRSTPTWCINT','INSHA',
                 ...
                 'iQCFullPelletCount','SINSTFIN','SINSTDET','tDetect','dRespMagThresh']
